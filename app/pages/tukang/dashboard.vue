    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="card p-4 text-center">
          ...
          <p class="text-2xl font-heading font-bold text-text-900">{{ stats.activeJobs }}</p>
          <p class="text-xs text-text-500">Pekerjaan Aktif</p>
        </div>

        <div class="card p-4 text-center">
          <div class="w-8 h-8 bg-success/10 rounded-full flex items-center justify-center mx-auto mb-2">
            <AwardIcon size="16" class="text-success" />
          </div>
          <p class="text-2xl font-heading font-bold text-text-900">{{ stats.certificates }}</p>
          <p class="text-xs text-text-500">Sertifikat</p>
        </div>

        <div class="card p-4 text-center">
          <div class="w-8 h-8 bg-accent/10 rounded-full flex items-center justify-center mx-auto mb-2">
            <StarIcon size="16" class="text-accent" />
          </div>
          <p class="text-2xl font-heading font-bold text-text-900">{{ stats.rating.toFixed(1) }}</p>
          <p class="text-xs text-text-500">Rating</p>
        </div>

        <div class="card p-4 text-center">
          <div class="w-8 h-8 bg-secondary/10 rounded-full flex items-center justify-center mx-auto mb-2">
            <TrendingUpIcon size="16" class="text-secondary" />
          </div>
          <p class="text-2xl font-heading font-bold text-text-900">{{ formatPrice(stats.monthlyEarnings) }}</p>
      ...
      <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          <!-- Profile Completion -->
          <div v-if="!user?.profileComplete" class="card p-6 border-l-4 border-l-accent">
            <div class="flex items-start space-x-4">
              ...
              <div class="flex-1">
                <h3 class="font-heading font-semibold text-lg text-text-900 mb-2">
                  Lengkapi Profil Anda
                </h3>
                <p class="text-text-500 mb-4">
                  Profil yang lengkap meningkatkan peluang mendapat pekerjaan hingga 5x lebih besar!
                </p>
                <div class="flex items-center justify-between mb-4">
                  <span class="text-sm text-text-500">Kelengkapan: {{ profileCompletion }}%</span>
            ...
            <div class="space-y-4">
              ...
              >
                <div class="flex items-start justify-between mb-3">
                  <div class="flex-1">
                    <h4 class="font-medium text-text-900 mb-1">{{ job.title }}</h4>
                  ...
                  <div class="text-right">
                    <p class="font-semibold text-primary">{{ formatPrice(job.budget) }}</p>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-xs text-success">{{ job.matchPercentage }}% cocok</span>
                </div>
              </div>
            </div>
              ...
              >
                <div
                  :class="[
                    ...
                    activity.type === 'training' ? 'bg-success/10 text-success' : '',
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-text-500">{{ formatRelativeTime(activity.createdAt) }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-6">
              ...
              >
                ...
                <div class="flex items-center justify-between text-xs text-text-500">
                  ...
                  <span class="text-success font-medium">
                    {{ training.price === 0 ? 'Gratis' : formatPrice(training.price) }}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div class="card p-6">
            ...
            <div class="space-y-3">
              <NuxtLink to="/tukang/portfolio" class="flex items-center p-3 rounded-lg hover:bg-gray-50 transition-colors">
      ...
      >
        ...
        <div class="divide-y divide-border">
          ...
          >
            ...
            <p class="text-xs text-text-500">{{ formatRelativeTime(notification.createdAt) }}</p>
          </div>
        </div>
      </div>
    </div>
...
const getActivityIcon = (type: string) => {
  switch (type) {
    ...
    case 'training': return 'BookOpenIcon'
    case 'certificate': return 'AwardIcon'
...
useHead({
  ...
  meta: [
    {
      name: 'description',
      content: 'Dashboard tukang profesional - kelola pekerjaan, pelatihan, dan sertifikat Anda'
    }
  ]
})
</script>
